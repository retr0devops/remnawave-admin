...
DEFAULT_CONFIG_DEFINITIONS: List[Dict[str, Any]] = [
    ...
    # === GEOIP ===
    {
        ...
        "description": "auto — GitHub release (mojolabs-id) → ltsdev mirror → MaxMind; github — GitHub release → ltsdev mirror; maxmind — только официальный (нужен ключ)",
        "default_value": "auto",
    ...
    {
        "key": "banhammer_support_contact",
        "value_type": "string",
        "category": "violations",
        "subcategory": "banhammer",
        "display_name": "Banhammer: контакт поддержки",
        "description": "Контакт поддержки, который добавляется в текст warning-уведомления",
        "default_value": "",
        "sort_order": 37,
    },
    {
        "key": "banhammer_block_message_template",
        "value_type": "string",
        "category": "violations",
        "subcategory": "banhammer",
        "display_name": "Banhammer: шаблон block-сообщения",
        "description": "Текст сообщения при блокировке пользователя. Поддерживает {ban_minutes}, {block_stage}, {support_contact}",
        "default_value": "Access is temporarily restricted for {ban_minutes} minute(s) due to node network policy mismatch.",
        "sort_order": 38,
    },
